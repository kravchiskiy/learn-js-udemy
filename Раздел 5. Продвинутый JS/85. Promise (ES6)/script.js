...
//промис который запускается при помощи fetch не перейдет в состояние отклонено т.е. rejected из-за ответа htp который считается ошибкой. (404, 500, 502).
//единственно что поменяется так это status в false. 
//простыми словами - если в фетче вернется промис попадает на ошибку связанная с http протоколом(404, 502), он не выкинет реджект. он нормально выполнит ресолв. самое главное, что он смог выполнить запрос в принципе. в следующем уроке разберем как это исправить. 